...
export default function ReportPage() {
  ...
  const reverseGeocode = async (lon: number, lat: number) => {
    ...
    try {
      const url = `https://api.maptiler.com/geocoding/${encodeURIComponent(`${lon},${lat}`)}.json?key=${process.env.NEXT_PUBLIC_MAPTILER_KEY}&language=en&limit=1`;
  ...
  // Forward geocode: from an address string -> coords + formatted label
  const forwardGeocode = async (query: string) => {
    if (!process.env.NEXT_PUBLIC_MAPTILER_KEY) return null;
    if (!query || query.trim().length === 0) return null;
    try {
      const url = `https://api.maptiler.com/geocoding/${encodeURIComponent(query)}.json?key=${process.env.NEXT_PUBLIC_MAPTILER_KEY}&language=en&limit=1`;
      const res = await fetch(url);
      const data = await res.json();
      const feat = data?.features?.[0];
      if (!feat) return null;
      const coords = (feat.geometry?.coordinates as [number, number]) || null;
      const label = feat.properties?.label || feat.place_name || feat.text || feat.properties?.name || null;
      if (!coords) return null;
      // coords are [lon, lat]
      return { lon: coords[0], lat: coords[1], label };
    } catch (err) {
  ...
  // Called by MapPicker when user selects a point on the map (MapPicker now reverse-geocodes itself for clicks)
  const pickCoords = async (coords: [number, number], address?: string | null) => {
    const [lon, lat] = coords;
    if (address) {
      // MapPicker provided a reverse-geocoded address -> use it directly
      setFormData((f) => ({ ...f, longitude: lon, latitude: lat, address }));
    } else {
      // fallback: do reverse geocode here if MapPicker didn't provide an address (rare)
  ...
  // Called when user types an address and presses Enter -> forward geocode
  const onAddressKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      e.preventDefault();
      ...
      if (res) {
        setFormData((f) => ({ ...f, longitude: res.lon, latitude: res.lat, address: res.label ?? q }));
        // MapPicker will receive the new initial via the props below and fly to it.
      } else {
        alert("Address not found. Try refining the address.");
      }
    }
  };
  ...
  const clearLocation = () => {
    setFormData((f) => ({ ...f, latitude: null, longitude: null, address: "" }));
  };
                  ...
                  <div className="space-y-4">
                    <div className="grid md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        />
                        <p className="text-xs text-muted-foreground mt-1">Type an address and press Enter to find it on the map.</p>
                      </div>
                      <div className="space-y-2">
                        ...
                        <div>
                          <Suspense fallback={<div className="w-full h-64 bg-muted rounded-md" />}>
                            <MapPicker
                              ...
                              onSelect={(coords, address) => pickCoords(coords, address)}
